...
var mainDraw = function() {
    ...
    var rebuildData = function(flag) {
        d3.json("data/data.json", function (classes) {
            ...
            var usersThres = $("#uThres").val();
            var papersThres = $("#pThres").val();
            var newData = [];
            var existing = {};
            var category, nUsers, nPapers, newItem;

            if(flag==0) {
                console.log("Updating by users");
                // add users that are within the threshold
                for (c in classes) {
                    category = classes[c].name.split(".")[0];
                    //if (category == "paper") console.log(classes[c].imports);
                    if (category == "user" && !(classes[c].name in existing) && classes[c].imports.length >= usersThres) {
                        newData.push(classes[c]);
                        existing[classes[c].name] = true;
                    }
                }
                nUsers = newData.length;
                    ...
                    for (i in newData[t].imports) {
                        for (c in classes) {
                            if (!(classes[c].name in existing) && classes[c].name == newData[t].imports[i]) {
                                newItem = jQuery.extend(true, {}, classes[c]);
                ...
                nPapers = newData.length - nUsers;
            }
            else {
                console.log("Updating by papers");
                // add users that are within the threshold
                for (c in classes) {
                    category = classes[c].name.split(".")[0];
                    //if (category == "paper") console.log(classes[c].imports);
                    if (category == "paper" && !(classes[c].name in existing) && classes[c].imports.length >= papersThres) {
                        newData.push(classes[c]);
                        existing[classes[c].name] = true;
                        existing[classes[c].name] = true;
                    }
                }
                nPapers = newData.length;
                // avoid inconsistency by adding each user in imports to newData
                for (t in newData) {
                    for (i in newData[t].imports) {
                        for (c in classes) {
                            if (!(classes[c].name in existing) && classes[c].name == newData[t].imports[i]) {
                                newItem = JSON.parse(JSON.stringify(classes[c]));
                                newItem.imports = []; // remove imports of papers (no extra users)
                                newData.push(newItem);
                                existing[classes[c].name] = true;
                            }
            ...
            d3.select("input[id=pThres]").on("input", pThresUpdate);
            ...
            function pThresUpdate() {
                $('#pThresL').text($(this).val());
                rebuildData(1);
            }
        });
    };
